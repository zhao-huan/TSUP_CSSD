...
class STFT(nn.Module):
    ...
    def forward(self, wavform):
        ...
        eps = torch.tensor(1e-10).to(device=device, dtype=dtype)
...
class Fbank(nn.Module):
    ...
    def forward(self, waveforms):
        assert len(waveforms.shape) in [2, 3], "input waveform must be 2 or 3 channels (batch, channel, time)"
        if len(waveforms.shape) == 2:
            waveforms = torch.unsqueeze(waveforms, 1)

        with torch.no_grad():
            power_spectrum, signal_log_energy = self.stft(waveforms)
...
def read_segments(segments_file, return_dict=True):
    results = {}
    with open(segments_file) as f:
        for line in f:
            sub_utt, utt, start, end = line.strip().split()
            results.setdefault(utt, []).append([sub_utt, float(start), float(end)])
        
        for key in results.keys():
            results[key] = sorted(results[key], key=lambda e:e[1])
    return results
